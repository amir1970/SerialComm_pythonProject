...
class Write_Read_Packet_Cls(object):
    ...
    def handle_read_opcode_0x3b(self,read_packet_obtained:bytearray,index_num:int,channel_request:int):  # , rx_pack_opcode_0x3b:bytearray):
        counts = bytes(bin(channel_request).count("1") for x in range(256))  # py2: use bytearray
        #find_pos:list = self.findPosition(channel_request)

        # find_pos = bin(channel_request)
        # for index_find_pos in find_pos:
        #     if (index_find_pos == 1):
        #         print("Found index_find_pos ", index_find_pos)
        #     else:
        #         print("Not Found index_find_pos ")

        #print("%%%  find position:  %%%", find_pos)
        print( "counts=",counts )

        filter_channel_num:int= int(0xE0)
        filter_meas:int= int(0x1F_FF_FF)
        mv=memoryview(read_packet_obtained)
        print("list of memory view= ", list(mv[0::]))
        tested_int_mv :int=0

        bytes_extracted = bytes(read_packet_obtained)
        print("bytes_extracted = ", bytes_extracted)
        ...
        for index_i in range(0,5):
            ...
            int_meas: int = int.from_bytes(read_packet_obtained[3*index_i+7:3*index_i+9], "big")
